...
const SuggestionsHandler = require('./suggestions_handler');

class OPENAI {
    constructor(token) {
        this.token = token;

        // Inicializar o handler de sugestões
        this.suggestionsHandler = new SuggestionsHandler(token);
        ...
        if (!token) {
            console.warn('⚠️  Token da OpenAI não fornecido - funcionando em modo limitado (apenas sugestões predefinidas)');
        }
    }
...
Análise técnica para especialista:`;
    ...
    // ===============================================
    // SUGESTÕES INTELIGENTES - NOVO SISTEMA
    // ===============================================

    async generateIntelligentSuggestions(analysis, weatherData, userContext) {
        try {
            console.log('🔄 Delegando geração de sugestões para SuggestionsHandler...');

            // Usar o novo sistema especializado
            const suggestions = await this.suggestionsHandler.generateSuggestions(
                analysis,
                weatherData,
                userContext
            );

            console.log('✅ Sugestões recebidas do handler:', suggestions);
            return suggestions;

        } catch (error) {
            console.error('❌ Erro no handler de sugestões:', error.message);
            return this.createFallbackSuggestions(analysis, weatherData);
        }
    }

    createFallbackSuggestions(analysis, weatherData) {
        // Garantir que analysis existe
        if (!analysis) {
            analysis = {
                type: "weather_data",
                city: "maputo",
        ...
        // Garantir que weatherData existe
        if (!weatherData) {
            weatherData = {
        ...
        // Sugestões baseadas nas condições climáticas - verificar se description existe
...
Para ajustar, digite "configurar alertas".`;
    ...
    // ===============================================
    // MÉTODOS PÚBLICOS PARA SUGESTÕES
    // ===============================================

    // Acesso direto ao handler de sugestões
    getSuggestionsHandler() {
        return this.suggestionsHandler;
    }

    // Gerar sugestões específicas para um contexto
    async getSuggestionsForContext(context, userLevel = 'basic') {
        return this.suggestionsHandler.getSuggestionsForContext(context, userLevel);
    }
    ...
    // Estatísticas do sistema de sugestões
    getSuggestionsStats() {
        return this.suggestionsHandler.getCacheStats();
    }

    // Testar conectividade do sistema de sugestões
    async testSuggestionsSystem() {
        return await this.suggestionsHandler.testConnection();
    }
    ...
    async processSuggestionResponse(suggestionText, weatherData, userContext = {}) {
        try {
            ...
            // Delegar para o handler especializado
            const result = await this.suggestionsHandler.processSuggestionResponse(
                suggestionText,
                weatherData,
                userContext
            );
            ...
            return {
                success: false,
                response: `❌ Erro ao processar "${suggestionText}". Tente novamente ou seja mais específico.`,
                suggestions: this.createFallbackSuggestions(null, weatherData),
                error: error.message
            };
        }
    }
